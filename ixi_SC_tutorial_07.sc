...
{EnvGen.ar(Env.adsr(0.001, 0.8, 0, 0.01, 1), Dust.ar(0.1)) *  SinOsc.ar}.play
...
(
{
	40.do({
		{ 	var freq, ratio, modulator, carrier;
			...
			ratio =   EnvGen.ar(Env.perc(0.05, 1)); // try this
...
(
SynthDef(\mygrain, {arg freq, amp, dur=1, t_trig;
	var signal, env;
	env = EnvGen.ar(Env.sine(dur, amp), t_trig, doneAction:0); 
	signal = SinOsc.ar(freq) * env;
	Out.ar(0, signal ! 2);
}).writeDefFile;
Server.local.sendMsg("/d_load", SynthDef.synthDefDir ++ "mygrain.scsyndef");
)
...
t = Task({
	...
	var dur = 0.01;
	var freq = 1/dur;
	a = Synth(\mygrain, [\dur, dur, \freq, 160, \amp, 0.5, \t_trig, 1]);
	freq.do({
		a.set(\t_trig, 1);
		dur.wait();
	});
...
b1.action = { arg view; 
	server.sendMsg("/d_load", "synthdefs/mygrain.scsyndef");
};
