...
SynthDef(\ks_guitar, { arg note, pan=0.0, rand, delayTime, noiseType=1;
	...
	freqs = [42, 77, 190, 210, 372, 511, 618];
	res = 5 * [0.0225, 0.0225, 0.025, 0.0225, 0.0125, 0.015, 0.0175];
	dec = 10 * [1, 1, 1, 1, 1, 1, 1];
	specs = [freqs, res, dec];
	signal = LPF.ar(signal, 2000);
	x = Klank.ar(`specs, signal);
...
SynthDef(\sin_grain_random, {arg freq, amp, grain_dur, dur=1, grain_freq, gate=1;
	...
	f = Drand([ 
		2677.777, 2700.0, 2728.0, 2766.666, 2800.0, 2850.0, 2900.0, 2950.0, 3000.0], inf);
...
a =	Synth(\ks_guitar, [\note, 82.0, \pan, 0, \rand, 0.15, \delayTime, 2+1.0.rand]);
